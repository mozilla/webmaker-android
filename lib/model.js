...
Model.prototype.isConnected = function () {
    var self = this;
    return [self._sync.SYNC_CONNECTED, self._sync.SYNC_CONNECTING].indexOf(self._sync.state) > -1;
};
...
Model.prototype.restore = function (callback) {
    ...
    self.readFileAsJson(self._ns, {defaultData: self.data}, function (file) {
        // Restore to model.data
    if (typeof file.history !== 'undefined') self.data.history = file.history;
        if (typeof file.apps !== 'undefined') self.data.apps = file.apps;
        if (typeof file.locale !== 'undefined') self.data.locale = file.locale;
        if (typeof file.offline !== 'undefined') self.data.offline = file.offline;
        ...
        if (self.data.offline || self.options.noConnect || global.location.search.match('offline=true') || config.OFFLINE) {
            self.offlineConnect();
        }
        if (self.options.noConnect) {
            return;
        ...
        function onLogin(user) {
            ...
            restorePath({
                history: self.data.history && model.data.history.path,
