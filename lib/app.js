app.jsvar Blocks = require('./blocks');
...
function App (id, ref) {
    ...
    self.firebase = ref || model.firebase.child(self.id);
}
...
App.createApp = function (options) {
    ...
    app.userId = model.data.session.user.id;
    app.author = model.data.session.user;
    ...
    return new App(app.id, ref);
};

App.prototype.update = function (properties) {
    var self = this;
    self.firebase.update(properties);
};
